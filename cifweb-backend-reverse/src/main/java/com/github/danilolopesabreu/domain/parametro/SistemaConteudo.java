 ...
 */
@Entity
@Table(name = "D02TB217_SISTEMA_CONTEUDO")
public class SistemaConteudo implements java.io.Serializable { private static final long serialVersionUID = 1L;

	private SistemaConteudoId id;
	private CifSrvcoPrpreFrmro cifSrvcoPrpreFrmro;
	private Date fimVigencia;
	private String coUsuario;

	public SistemaConteudo() {
	}

	public SistemaConteudo(SistemaConteudoId id, CifSrvcoPrpreFrmro cifSrvcoPrpreFrmro, String coUsuario) {
		this.id = id;
		this.cifSrvcoPrpreFrmro = cifSrvcoPrpreFrmro;
		this.coUsuario = coUsuario;
	}

	public SistemaConteudo(SistemaConteudoId id, CifSrvcoPrpreFrmro cifSrvcoPrpreFrmro, Date fimVigencia,
			String coUsuario) {
		this.id = id;
		this.cifSrvcoPrpreFrmro = cifSrvcoPrpreFrmro;
	...
	@AttributeOverrides({
			...
			@AttributeOverride(name = "nuSegmentoS", column = @Column(name = "NU_SEGMENTO_S12", nullable = false, precision = 9, scale = 0)),
			@AttributeOverride(name = "inicioVigencia", column = @Column(name = "DT_INICIO_VIGENCIA", nullable = false, length = 7)) })
	public SistemaConteudoId getId() {
		return this.id;
	}

	public void setId(SistemaConteudoId id) {
		this.id = id;
	}
